...
# Keywords that force CRITICAL regardless of what the model returns. This is
# the "rule engine has priority over the LLM" safety layer -- deliberately
# simple for the build window, but it demonstrates the principle: the LLM
# explains and structures, it never gets the final say on an emergency.
...
def chat(
    ...
    if message:
        ...
        session.history.append(Turn(role="user", text=message))
        session.history.append(
            Turn(role="assistant", text=result.explanation, structured=result)
        )
